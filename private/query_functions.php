<?php
  ...
  // let's make them use a union instead!
  // function secretsOutForHarambe() {
  //   global $db;
  //   $sql = "";
  //   $secretResult = db_query($db, $sql);
  //   return $secretResult;
  // }
  ...
  function count_failed_logins($username) {
    global $db;
    $sql = "SELECT count(*) FROM failed_logins ";
    $sql .= "WHERE username='" . db_escape($db, $username) . "';";
    $count = db_query($db, $sql);
    return $count;
  }
  ...
  // ruh roh, Dark Shadow wuz here
  function find_territories_by_name_vulnerable($name="") {
    global $db;
    $sql = "SELECT name, state_id FROM territories ";
